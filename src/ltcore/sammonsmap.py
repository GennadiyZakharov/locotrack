'''
Created from https://raw.githubusercontent.com/tompollard/sammon/master/sammon.py
'''
...
def sammon(x, n = 2, display = 2, inputdist = 'raw', maxhalves = 20, maxiter = 500, tolfun = 1e-9, init = 'pca'):
    ...
    def calculateDistances(arr):
        nSamples=len(arr)
        distances=np.zeros((nSamples,nSamples))
        for i in range(nSamples):
            for j in range(i+1,nSamples):
                distances[i,j]=distance(arr[i],arr[j])
    ...
    one = np.ones([N,n])
    d = calculateDistances(y) + np.eye(N)
    dinv = 1. / d # Returns inf where d = 0. 
    dinv[np.isinf(dinv)] = 0 # Fix by replacing inf with 0 (default Matlab behaviour).
    delta = D-d 
    E = ((delta**2)*Dinv).sum() 
